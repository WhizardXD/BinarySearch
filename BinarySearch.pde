...
public int recursiveBinarySearch(int catNumToFind, int nLow, int nHigh) {
  //complete this method
  int num = 0;
  int midGuess = (nLow+nHigh)/2;
  if (nLow > nHigh) {
    return -1;
  }
  if (store[midGuess].getCatNum() == catNumToFind) {
    return store[midGuess].getInventory();
  } else if (store[midGuess].getCatNum() > catNumToFind) {
    nHigh = midGuess-1;
    return recursiveBinarySearch(catNumToFind,nLow,nHigh);
  } else {
    nLow = midGuess+1;
    return recursiveBinarySearch(catNumToFind,nLow,nHigh);
  }
